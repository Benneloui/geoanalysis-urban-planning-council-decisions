...
get_location_patterns <- function() {
  list(
    ...
    address = "([A-ZÄÖÜ][a-zäöüß]+(?:straße|str\\.|weg|platz|allee|gasse|ring)\\s+\\d+[a-z]?)",

    # Parcel number pattern (e.g., "Fl.Nr. 123", "Flurnummer 456/7")
    parcel = "(Fl(?:urnummer|\\.\\s?Nr\\.?)\\s*\\d+(?:/\\d+)?)",

    # Building plan pattern - use the string pattern, not the regex object
    bplan = "(bebauungsplan|bebauungspl[aä]n[e]?|b-?plan|bpl[aä]n[e]?)"
  )
}
